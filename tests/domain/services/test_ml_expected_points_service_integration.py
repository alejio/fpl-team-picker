...
2. Feature engineer handles missing betting data gracefully (neutral defaults)
...
class TestFPLFeatureEngineer99Features:
    ...
    def test_feature_engineer_produces_99_features_with_betting_odds(
        ...
        betting_features = [
            "team_win_probability", "opponent_win_probability", "draw_probability",
            "implied_clean_sheet_probability", "implied_total_goals", "team_expected_goals",
            "market_consensus_strength", "odds_movement_team", "odds_movement_magnitude",
            "favorite_status", "asian_handicap_line", "handicap_team_odds",
            "expected_goal_difference", "over_under_signal", "referee_encoded"
        ]
        for feature in betting_features:
            assert feature in result.columns, f"Missing betting feature: {feature}"

    def test_feature_engineer_produces_99_features_without_betting_data(
        self,
        sample_historical_data,
        sample_teams_data,
        sample_fixtures_data,
        ...
        """Test that FPLFeatureEngineer produces 99 features even without betting data (uses neutral defaults)."""
        from fpl_team_picker.domain.services.ml_feature_engineering import FPLFeatureEngineer
        ...
        engineer = FPLFeatureEngineer(
            ...
            value_analysis_df=sample_value_analysis,
            fixture_difficulty_df=sample_fixture_difficulty,
            betting_features_df=None,  # No betting data
        )
        ...
        # Should still have 99 features (with neutral defaults)
        assert result.shape[1] == 99, f"Expected 99 features, got {result.shape[1]}"

        # Betting features should exist with neutral values
        assert "team_win_probability" in result.columns
        assert all(np.isclose(result["team_win_probability"], 0.33, atol=0.01))
        assert all(result["asian_handicap_line"] == 0.0)
        assert all(result["referee_encoded"] == -1)
