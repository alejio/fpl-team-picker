...
class MLXPModel:
    ...
    def prepare_training_data(self, live_data_df: pd.DataFrame, target_gw: int) -> Tuple[pd.DataFrame, List[str]]:
        """
        ...
        try:
            ...
            historical_gws = sorted([gw for gw in live_data_df['event'].unique() if gw < target_gw])
            
            if len(historical_gws) < self.min_training_gameweeks:
                raise ValueError(f"Need at least {self.min_training_gameweeks} historical gameweeks, found {len(historical_gws)}")
            
            # Use recent gameweeks for training
            train_gws = historical_gws[-self.training_gameweeks:] if len(historical_gws) >= self.training_gameweeks else historical_gws
            ...
            available_enhanced_cols = [col for col in self.enhanced_features_cols if col in enhanced_players.columns]
                ...
                if not historical_data_up_to_gw.empty:
                    ...
                    for enhanced_col in available_enhanced_cols:
                        cumulative_stats[f'{enhanced_col}_static'] = cumulative_stats['player_id'].map(
                            player_enhanced_features[enhanced_col] if enhanced_col in player_enhanced_features.columns 
                            else pd.Series(dtype=float)
                        ).fillna(0 if enhanced_col != 'chance_of_playing_next_round' else 100)
                    
                    # Prepare historical features
                    historical_features = cumulative_stats[['player_id', 'position', 'xG90_historical', 'xA90_historical', 'points_per_90']].rename(columns={
                        'bps': 'bps_historical',
                    ...
                    for enhanced_col in available_enhanced_cols:
                        ...
                        if static_col in cumulative_stats.columns:
                            historical_features[static_col] = cumulative_stats[static_col]
                    ...
                    if not prev_gw_data.empty:
                        # Get lagged features
                        available_cols = ['player_id'] + [col for col in ['minutes', 'goals_scored', 'assists', 'bonus'] if col in prev_gw_data.columns]
                        ...
                        for col in ['prev_minutes', 'prev_goals', 'prev_assists', 'prev_bonus', 'prev_goals_per_90', 'prev_assists_per_90', 'prev_bonus_per_90']:
                            gw_features[col] = 0
            ...
            # Feature engineering
            self.label_encoder = LabelEncoder()
            training_data['position_encoded'] = self.label_encoder.fit_transform(training_data['position'])
            
            # Define feature set
            base_features = [
                'position_encoded', 'xG90_historical', 'xA90_historical', 'points_per_90'
            ]
            
            # Add enhanced static features
            enhanced_static_features = [f'{col}_static' for col in available_enhanced_cols]
            ...
            # Clean data
            training_data = training_data.dropna(subset=ml_features + [self.target_variable])
            
            if self.debug:
                logger.debug(f"Training data prepared: {len(training_data)} samples, {len(ml_features)} features")
        ...
        try:
            ...
            for position in ['GKP', 'DEF', 'MID', 'FWD']:
                ...
                if len(pos_data) >= self.position_min_samples:
                    ...
                    alpha_range = [10.0, 100.0, 1000.0] if position in ['GKP', 'FWD'] else [0.1, 1.0, 10.0, 100.0]
                    pos_model = RidgeCV(
                        alphas=alpha_range,
                    ...
                    if self.debug:
                        logger.debug(f"{position}: Train MAE {pos_train_mae:.2f}, Test MAE {pos_test_mae:.2f}")
                else:
                    metrics['position_specific'][position] = {
            ...
            if self.debug:
                logger.debug(f"General model: Train MAE {general_train_mae:.2f}, Test MAE {general_test_mae:.2f}")
                logger.debug(f"Trained {len(self.position_models)} position-specific models")
    ...
    def predict(self, current_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            available_enhanced_pred = [col for col in self.enhanced_features_cols if col in enhanced_players_current.columns]
                ...
                if feature not in current_data.columns:
                    if 'position_encoded' in feature:
                        ...
                        if 'position' in current_data.columns:
                            current_data['position_encoded'] = self.label_encoder.transform(current_data['position'].fillna('MID'))
                        else:
    ...
    def calculate_expected_points(self, 
                                ...
                                rule_based_model=None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            training_data, ml_features = self.prepare_training_data(live_data, target_gameweek)
            ...
            if rule_based_model is not None and self.ensemble_rule_weight > 0:
                try:
                    ...
                    predictions_df['xP'] = (ml_weight * predictions_df['ml_ensemble_xP'] + 
                                          self.ensemble_rule_weight * rule_predictions['xP'])
                    
                    if self.debug:
                        logger.debug(f"Created ensemble with ML weight {ml_weight:.2f}, Rule weight {self.ensemble_rule_weight:.2f}")
                        
                except Exception as e:
                    logger.warning(f"Rule-based ensemble failed, using pure ML: {str(e)}")
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        importance_df = pd.DataFrame({
            'feature': self.ml_features,
            'importance': abs(self.general_model.coef_)
    ...
    try:
        ...
        if 'form_multiplier' not in merged.columns:
            ...
            if 'xP' in merged.columns:
                ...
                merged.loc[merged['xP'] >= xp_percentiles[0.8], 'form_multiplier'] = 1.3  # Hot
                merged.loc[merged['xP'] <= xp_percentiles[0.2], 'form_multiplier'] = 0.7  # Cold
        
        if 'recent_points_per_game' not in merged.columns:
            # Estimate recent PPG from xP (simplified)
            merged['recent_points_per_game'] = merged.get('xP', 0) * 0.8  # Conservative estimate
        ...
        if 'form_trend' not in merged.columns:
            ...
            merged['form_trend'] = merged['recent_points_per_game'] - 4.0  # 4.0 as baseline
