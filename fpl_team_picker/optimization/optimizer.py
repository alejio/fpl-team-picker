...
def calculate_total_budget_pool(current_squad: pd.DataFrame, bank_balance: float, players_to_keep: Optional[Set[int]] = None) -> Dict:
    ...
    sellable_value = sellable_players['price'].sum() if not sellable_players.empty else 0.0
...
def premium_acquisition_planner(current_squad: pd.DataFrame, all_players: pd.DataFrame, budget_pool_info: Dict, top_n: int = 3) -> List[Dict]:
    """
    ...
    for _, target in premium_targets.iterrows():
        ...
        for _, player in potential_sells.iterrows():
            ...
            if cumulative_value >= funding_gap:
                ...
                min_replacement_cost = remaining_slots * 4.0  # Assume £4m minimum per replacement
                
                if budget_pool_info['total_budget'] - target['price'] >= min_replacement_cost:
                    # Find cheap replacements for sold players
                    replacements = []
                    remaining_budget = budget_pool_info['total_budget'] - target['price']
...
def get_best_starting_11(squad_df: pd.DataFrame, xp_column: str = 'xP') -> Tuple[List[Dict], str, float]:
    ...
    for pos in by_position:
        by_position[pos].sort(key=lambda p: p.get(sort_col, p.get('xP', 0)), reverse=True)
